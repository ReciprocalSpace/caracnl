...
#             φ = mat['S11_comp_smi'][:, 2] / 180 * pi  # Phase [rad]
...
def _polar_to_complex(norm_in_dBm, angle_in_degree):
    return 10 ** (norm_in_dBm / 20) * np.exp(1j * angle_in_degree * np.pi / 180)


def get_s11_data_from_file(DirOI, sans_ampli=True):
    if sans_ampli:
        # Liste des fichiers
        SubdirOI, FilesOI, ExpParam_OI = [], [], []
        for (dirpath, dirnames, filenames) in walk(DirOI):
            for dirname in dirnames:
                SubdirOI.append(dirname)
            SubdirOI.sort()
        ...
        # calibration du pont de reflectométrie
        S21_Short_File, S21_50Ohm_File, S21_Open_File = [], [], []
        S31_Short_File, S31_50Ohm_File, S31_Open_File = [], [], []
        for (dirpath, dirnames, filenames) in walk(DirOI):
            files = filenames
            for f in files:
                if f[-18:] == '_S21_smi_short.mat':
                    S21_Short_File.append(path.join(dirpath, f))
                elif f[-18:] == '_S31_smi_short.mat':
                    S31_Short_File.append(path.join(dirpath, f))
                elif f[-18:] == '_S21_smi_50Ohm.mat':
                    S21_50Ohm_File.append(path.join(dirpath, f))
                elif f[-18:] == '_S31_smi_50Ohm.mat':
                    S31_50Ohm_File.append(path.join(dirpath, f))
                elif f[-17:] == '_S21_smi_open.mat':
                    S21_Open_File.append(path.join(dirpath, f))
                elif f[-17:] == '_S31_smi_open.mat':
                    S31_Open_File.append(path.join(dirpath, f))

        mat = loadmat(S21_Short_File[0])
        S21_Short_Data = mat['S21_smi_short']

        mat = loadmat(S31_Short_File[0])
        S31_Short_Data = mat['S31_smi_short']

        mat = loadmat(S21_50Ohm_File[0])
        S21_50Ohm_Data = mat['S21_smi_50Ohm']

        mat = loadmat(S31_50Ohm_File[0])
        S31_50Ohm_Data = mat['S31_smi_50Ohm']

        mat = loadmat(S21_Open_File[0])
        S21_Open_Data = mat['S21_smi_open']

        mat = loadmat(S31_Open_File[0])
        S31_Open_Data = mat['S31_smi_open']

        S31_tot_V_50Ohm = _polar_to_complex(S31_50Ohm_Data[:, 1], S31_50Ohm_Data[:, 2])
        S21_tot_V_50Ohm = _polar_to_complex(S21_50Ohm_Data[:, 1], S21_50Ohm_Data[:, 2])
        S31_tot_V_open = _polar_to_complex(S31_Open_Data[:, 1], S31_Open_Data[:, 2])
        S21_tot_V_open = _polar_to_complex(S21_Open_Data[:, 1], S21_Open_Data[:, 2])
        S31_tot_V_short = _polar_to_complex(S31_Short_Data[:, 1], S31_Short_Data[:, 2])
        S21_tot_V_short = _polar_to_complex(S21_Short_Data[:, 1], S21_Short_Data[:, 2])

        rho1 = S31_tot_V_50Ohm / S21_tot_V_50Ohm
        rho2 = S31_tot_V_short / S21_tot_V_short
        rho3 = S31_tot_V_open / S21_tot_V_open
        ...
        # Calcul S11
        FilesOI, S21FilesOI, S31FilesOI = [], [], []
        ExpParam_OI, SubdirOI = [], []
        for (dirpath, dirnames, filenames) in walk(DirOI):
            for dirname in dirnames:
                SubdirOI.append(dirname)
            SubdirOI.sort()
        ...
        for subdir in SubdirOI:
            for (sub_dirpath, sub_dirnames, sub_filenames) in walk(path.join(DirOI, subdir)):
                for f in sub_filenames:
                    if f[-12:] == '_S21_smi.mat':
                        S21FilesOI.append(path.join(sub_dirpath, f))
                    if f[-12:] == '_S31_smi.mat':
                        S31FilesOI.append(path.join(sub_dirpath, f))
                    if f[-14:] == '_Param_exp.txt':
                        ExpParam_OI.append(path.join(sub_dirpath, f))
